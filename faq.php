...
<?php generateQuestionComponent("Czy koniecznie trzeba mieć receptę, żeby zamówić okulary?", "Nie.
...
<?php generateQuestionComponent("Ile się czeka na okulary?","Czas oczekiwania zależy od rodzaju szkieł, które zostaną wybrane.
...
<?php generateQuestionComponent("Co będzie jeżeli się nie przyzwyczaję do okularów progresywnych?","W tej sytuacji prosimy o niezwłoczny kontakt z optykiem, w którym one zostały zrobione w celu ustalenia przyczyny braku komfortu.
...
<?php generateQuestionComponent("Czy istnieje możliwość zbadania wzroku u dzieci?","  W naszych salonach niestety takiej możliwości nie ma.
...
<?php generateQuestionComponent("Ile kosztują szkła progresywne?","  Cena szkieł progresywnych jest mocno uzależniona od stopnia zaawansowania konstrukcji soczewki, niezbędnego stopnia redukcji grubości oraz innych uszlachetnień.
...
<?php generateQuestionComponent("Czy są dostępne okulary do komputera?", "  Gotowych tzw “zerówek” nie mamy w ofercie.
...
<?php generateQuestionComponent("Jakie okulary wybrać do samochodu?","  Przeciwsłoneczne okulary warto wybrać z powłoką polaryzacyjną.
...
<?php generateQuestionComponent("Jak się przygotować do badania wzroku?","Wziąć poprzednie okulary i receptę
...
<?php generateQuestionComponent("Czy można naprawić okulary?","  Pewne uszkodzenia możemy naprawić. W celu ustalenia czy w danym przypadku jesteśmy w stanie pomóc prosimy o kontakt z nami: przesłanie zdjęcia uszkodzenie MMSem lub przez media społecznościowe.
...
<?php generateQuestionComponent("Czym jest antyrefleks?","  Jest to powłoka na soczewkach okularowych, która robi je bardziej przyjżyste. Każdy producvent szkieł ma w ofercie takie szkła
...
<?php generateQuestionComponent("Czy można wymienić szkłą w swoich okularach?", "  Wymiana szkieł do własnej oprawy jest możliwa. Prosimy o pokazanie oprawki w celu ustalenia czy jest możliwa wymiana szkieł w danej oprawce. 
